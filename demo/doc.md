...
```
 Python / Swift / Kotlin / ...       (language wrapper: ~3,000 lines each)
            |
       Rust host + wasmtime           (WASM runtime, WASI HTTP, FFI)
            |
       WASM component                 (compiled TS SDK + AWS SDK + model providers)
            |
       WIT contract                   (agent.wit: the typed API boundary)
```
...
> Note: After writing this doc, I did actually implement Strands in Kotlin, providing support for both Java and Kotlin at the same time. This projection has been proven.
...
Rust is required because the wasmtime C API does not expose WASI HTTP, which the WASM component needs to call model provider APIs. The Rust layer is ~1,500 lines and changes infrequently. Once the host is built, most development happens in TypeScript for SDK features or in the target language for wrapper glue. A developer adding a model provider or fixing an agent loop bug writes TypeScript, not Rust.

**How do you debug across the WASM boundary?**

Logging crosses the boundary through the `host-log` WIT import. The guest emits structured log entries that the host routes to the language's logging framework. For deeper debugging, `use_jit` mode loads the uncompiled WASM at runtime, which is slower but allows inspecting the full stack. Most bugs are either in the TypeScript SDK, where they can be debugged with normal TS tooling before compilation, or in the language wrapper, where they can be debugged natively.
...
The polyglot Python wrapper replaces the internals, not the API. Users see the same `Agent` class, the same `@tool` decorator, the same imports. The wrapper passes 54 of 106 upstream integration tests unmodified today. The goal is full compatibility.

**Will development be harder?**

The build pipeline has more layers, but the demo includes a dev CLI (`strands-dev`) that handles the full pipeline in one command. Day to day, most feature work happens in one language instead of two, bugs get fixed once, and the WIT contract catches integration mistakes at compile time. The rigor comes from the architecture rather than from discipline across independent codebases.
...
After writing this document, I built a Kotlin wrapper using the Rust host's UniFFI bindings. It works in both Kotlin and Java. The "adding a new language" section was written as a projection; it is now a measured result.
...
| Test                                                                                            | Status        | Notes                                                                                                           |
| ----------------------------------------------------------------------------------------------- | ------------- | --------------------------------------------------------------------------------------------------------------- |
| `hooks/test_events::test_events`                                                                | FAILED        | Lifecycle event ordering — bridge batches events at `readNext()` boundaries                                     |
...
| `mcp/test_mcp_client::test_mcp_client`                                                          | FAILED        | Message format — `agent.messages` missing `toolUse` blocks after SSE tool invocation                            |
| `mcp/test_mcp_client::test_mcp_client_async_structured_content`                                 | PASSED        |                                                                                                                 |
| `mcp/test_mcp_client::test_mcp_client_connection_stability_with_client_timeout`                 | FAILED        | Error message logging mismatch — test mocks `_log_debug_with_thread`                                            |
...
| `mcp/test_mcp_client::test_streamable_http_mcp_client_with_500_error`                           | FAILED        | Thread cleanup after server 500 — background thread not fully cleaned up                                        |
...
| `mcp/test_mcp_client_tasks::test_server_capabilities`                                           | FAILED        | Task-augmented execution API — `_server_task_capable` attribute check                                           |
...
| `models/test_model_bedrock::test_document_citations`                                            | FAILED        | Citations content block not in TS SDK                                                                           |
| `models/test_model_bedrock::test_document_citations_streaming`                                  | FAILED        | Citations content block not in TS SDK                                                                           |
...
| `models/test_model_bedrock::test_non_streaming_model_events`                                    | FAILED        | Test expects `model.stream()` method not on our model wrapper                                                   |
| `models/test_model_bedrock::test_reasoning_content_in_messages_with_thinking_disabled`          | FAILED        | Reasoning content block not mapped through WASM bridge                                                          |
| `models/test_model_bedrock::test_redacted_content_handling`                                     | FAILED        | Redacted content block not mapped through WASM bridge                                                           |
| `models/test_model_bedrock::test_streaming_agent`                                               | PASSED        |                                                                                                                 |
| `models/test_model_bedrock::test_streaming_model_events`                                        | FAILED        | Test expects `model.stream()` method not on our model wrapper                                                   |
...
| `test_bedrock_cache_point::test_bedrock_cache_point`                                            | FAILED        | `cache_read_input_tokens` not exposed — `callback_handler` not wired to stream events                           |
| `test_bedrock_cache_point::test_bedrock_multi_prompt_and_duplicate_cache_point`                 | FAILED        | `cache_read_input_tokens` not exposed — `callback_handler` not wired to stream events                           |
| `test_bedrock_guardrails::test_guardrail_input_intervention[disabled]`                          | XFAIL         | Expected failure                                                                                                |
| `test_bedrock_guardrails::test_guardrail_input_intervention[enabled]`                           | FAILED        | TS SDK has no guardrail config — only maps stop reason                                                          |
...
| `test_bedrock_s3_location::test_document_s3_location`                                           | FAILED        | Content format mismatch (`source.location` vs `source.s3Location`)                                              |
...
| `test_context_overflow::test_context_window_overflow`                                           | FAILED        | Model context window large enough to handle test payload                                                        |
...
| `test_invalid_tool_names::test_invalid_tool_names_works`                                        | FAILED        | Agent gets 4 messages instead of 6 — model retry on invalid tool name doesn't produce second assistant response |
| `test_max_tokens_reached::test_max_tokens_reached`                                              | PASSED        |                                                                                                                 |
| `test_session::test_agent_with_file_session`                                                    | FAILED        | `FileSessionManager` is a no-op stub                                                                            |
...
| `test_session::test_agent_with_s3_session`                                                      | FAILED        | `S3SessionManager` is a no-op stub                                                                              |
...
| `tools/test_thread_context::test_agent_invoke_context_sharing`                                  | FAILED        | Threading context variable not propagating across agent invocation boundary                                     |
| `tools/test_thread_context::test_tool_call_context_sharing`                                     | PASSED        |                                                                                                                 |
| `a2a/test_multiagent_a2a`                                                                       | NOT COLLECTED | Gets past `a2a.client` but then `strands.agent.a2a_agent` — not implemented                                     |
| `bidi/test_bidi_hooks`                                                                          | NOT COLLECTED | `strands.experimental.bidi` — no TS SDK equivalent                                                              |
| `bidi/test_bidirectional_agent`                                                                 | NOT COLLECTED | `strands.experimental.bidi` — no TS SDK equivalent                                                              |
| `bidi/tools/test_direct`                                                                        | NOT COLLECTED | `strands.experimental.bidi` — no TS SDK equivalent                                                              |
| `hooks/multiagent/test_cancel`                                                                  | NOT COLLECTED | `strands.types._events.MultiAgentNodeCancelEvent` — upstream-only internal type                                 |
| `hooks/multiagent/test_events`                                                                  | NOT COLLECTED | `strands.hooks.AfterMultiAgentInvocationEvent` — upstream-only hook events                                      |
| `models/test_conformance`                                                                       | NOT COLLECTED | Shared `providers.py` imports `strands.models.litellm` — no TS SDK provider                                     |
| `models/test_model_cohere`                                                                      | NOT COLLECTED | Shared `providers.py` imports `strands.models.litellm` — no TS SDK provider                                     |
| `models/test_model_gemini`                                                                      | NOT COLLECTED | Shared `providers.py` imports `strands.models.litellm` — no TS SDK provider                                     |
| `models/test_model_litellm`                                                                     | NOT COLLECTED | `strands.models.litellm` — no TS SDK provider                                                                   |
| `models/test_model_llamaapi`                                                                    | NOT COLLECTED | `strands.models.llamaapi` — no TS SDK provider                                                                  |
| `models/test_model_llamacpp`                                                                    | NOT COLLECTED | `strands.models.llamacpp` — no TS SDK provider                                                                  |
| `models/test_model_mistral`                                                                     | NOT COLLECTED | `strands.models.mistral` — no TS SDK provider                                                                   |
| `models/test_model_ollama`                                                                      | NOT COLLECTED | `strands.models.ollama` — no TS SDK provider                                                                    |
| `models/test_model_openai`                                                                      | NOT COLLECTED | Shared `providers.py` imports `strands.models.litellm` — no TS SDK provider                                     |
| `models/test_model_sagemaker`                                                                   | NOT COLLECTED | `strands.models.sagemaker` — no TS SDK provider                                                                 |
| `models/test_model_writer`                                                                      | NOT COLLECTED | `strands.models.writer` — no TS SDK provider                                                                    |
| `steering/test_model_steering`                                                                  | NOT COLLECTED | `strands.experimental.steering` — no TS SDK equivalent                                                          |
| `steering/test_tool_steering`                                                                   | NOT COLLECTED | `strands.experimental.steering` — no TS SDK equivalent                                                          |
| `test_a2a_executor`                                                                             | NOT COLLECTED | `strands.multiagent.a2a` — A2A server not implemented                                                           |
| `test_agent_json`                                                                               | NOT COLLECTED | `strands.experimental.config_to_agent` — no TS SDK equivalent                                                   |
